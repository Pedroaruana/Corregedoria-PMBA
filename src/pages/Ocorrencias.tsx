...
const STATUS_FILTERS = ['Todos', 'Registrada', 'Em Análise', 'Aguardando Laudo IML', 'Aguardando Assinatura', 'Concluída']
...
export function Ocorrencias() {
  ...
  function nomesPolicial(policiais: OcorrenciaAPI['policiais']) {
    return policiais.map(p => `${p.patente}. ${p.nome.split(' ')[0]}`).join(' / ')
  }

  const filtradas = ocorrencias.filter(o => {
    const local = `${o.bairro}, ${o.municipio}`
    const policiaisStr = nomesPolicial(o.policiais)
    const matchBusca =
      o.protocolo.toLowerCase().includes(busca.toLowerCase()) ||
      local.toLowerCase().includes(busca.toLowerCase()) ||
      policiaisStr.toLowerCase().includes(busca.toLowerCase())
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              ...
              filtradas.map(o => (
                <tr key={o.protocolo} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-600">
                    {formatarData(o.dataFato)}<br />
                    <span className="text-xs text-gray-400">{o.horaFato}</span>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{o.bairro}, {o.municipio}</td>
                  <td className="px-4 py-3 text-gray-600">{o.bpm}</td>
                  <td className="px-4 py-3 text-gray-600">{nomesPolicial(o.policiais)}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-semibold px-2 py-1 rounded ${statusColors[o.status] ?? 'bg-gray-100 text-gray-600'}`}>
                      {o.status}
                    </span>
                  </td>
                  <td className="px-4 py-3">
        ...
        <div className="px-4 py-3 border-t border-gray-100 bg-gray-50 text-xs text-gray-400">
          {loading ? '...' : `${filtradas.length} de ${ocorrencias.length} registros`}
        </div>
      </div>
    </div>
  )
